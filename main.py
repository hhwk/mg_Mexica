...
if menu=='Гуманитарная помощь':
    ...
    visit_money = st.selectbox('Кому вы хотите перевести деньги?',('Австралии', 'Канаде', 'Филипинским островам', 'Швеции', 'Аргентине'))
...
if menu=='Посещения':
    visit = st.selectbox('Какую старану вы хотите посетить?', ('Австралия', 'Канада', 'Филипинские острова', 'Швеция', 'Аргентина'))
    ...
    if city['reserch']=='0':
        ...
        for l in range(0,len(country)):
            if country[l]=='Австралия':
                attak=st.multiselect('Какие города атакуем в Австралия?',['Канберра','Сидней','Мельбрун','Перт'])
            if country[l]=='Канада':
...
if menu=='Улучшения':
    ...
    col1.metric('🏠'+city['shit1']+masiv_shit[0]+f'{city_1}','⚙️'+str(0)+'%'+' 🌳 '+str(0)+'%',masiv_up[0]*10)
...
if menu=='Стартовая страница':
    st.title(f'Вы играете за Мексику')
    st.write('Деньги:', money)
    ...
    if eco4 > 100:
        eco4 = 100
    col1, col2, col3, col4 = st.columns(4)
    col1.metric('🏠' + city['shit1'] + f'{city_1}','⚙️' + str(0) + '%' + ' 🌳 ' + str(0) + '%')
